...
export default function TicketsEdit() {
  ...
  // 공연명
  const [ticketName, setTicketName] = useState(ticketData.title); // 사용
  const ticketNameRef = useRef();
  // 장소
  const [gu, setGu] = useState(ticketData.address); // 사용
  const [place, setPlace] = useState(newAdress[0]); // 사용
  const placeRef = useRef();
  const [detailPlace, setDetailPlace] = useState(newAdress[1]); // 사용
  const detailPlaceRef = useRef();

  // 공연 시작 정보
  const [startDate, setStartDate] = useState(ticketData.showAt); // 사용
  const [endDate, setEndDate] = useState(ticketData.expiredAt); // 사용
  const [startTime, setStartTime] = useState(ticketData.showAt); // 사용
  const startTimeRef = useRef();

  // 공연 좌석 수
  const [sit, setSit] = useState(ticketData.total);
  const sitRef = useRef();
  // 티켓 가격
